...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        // DataBox splitKey = rootSplit.get().getFirst();
        // newKeys.add(splitKey);
        // newChildren.add(root.getPage().getPageNum());
        // newChildren.add(1, rootSplit.get().getSecond());
        //
        // BPlusNode newRoot = new InnerNode(metadata, bufferManager, newKeys,
        // newChildren, lockContext);
        // updateRoot(newRoot);

        // tom's implementation
        Optional<Pair<DataBox, Long>> newPair = root.put(key, rid);
        ...
        Pair<DataBox, Long> splitPair = newPair.get();
        keys.add(splitPair.getFirst());
        children.add(root.getPage().getPageNum());
        children.add(splitPair.getSecond());
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (this.root == this.root.getLeftmostLeaf() &&
                !this.root.getLeftmostLeaf().scanAll().hasNext()) {
            while (data.hasNext()) {
                Optional<Pair<DataBox, Long>> bl = this.root.bulkLoad(data, fillFactor);
                if (bl.isPresent()) {
                    List<DataBox> keys = new ArrayList<>();
                    List<Long> children = new ArrayList<>();
                    keys.add(bl.get().getFirst());
                    children.add(root.getPage().getPageNum());
                    children.add(bl.get().getSecond());
                    BPlusNode newRoot = new InnerNode(metadata, bufferManager, keys, children, this.lockContext);
                    this.root = newRoot;
                }
            }
        } else {
            throw new BPlusTreeException("Tree is not empty, can't bulkload");
        }
    }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator(BPlusNode root, DataBox key) {
            ...
            currIter = currLeaf.scanGreaterEqual(key);
        }

        @Override
        public boolean hasNext() {
            // TODO(proj2): implement
            return (currIter.hasNext() || (currLeaf.getRightSibling().isPresent()
